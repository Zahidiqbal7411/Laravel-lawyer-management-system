        ...
        <div class="page-header-content d-lg-flex border-top">
            <div class="d-flex">
                <div class="breadcrumb py-2">
                    ...
                    <a href="{{ route(getPrefix() .'assign_roles.index') }}" class="breadcrumb-item">Assigned Roles</a>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">Edit Role Assignment</h5>
                    </div>
                    ...
                    <div class="card-body border-top">
                        <form action="{{ route(getPrefix() .'assign_roles.store') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <select name="user_id" id="user_id"
                                        ...
                                        @foreach ($users as $userItem)
                                            <option value="{{ $userItem->id }}"
                                                {{ (old('user_id') ?? $user->id) == $userItem->id ? 'selected' : '' }}>
                                                {{ $userItem->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-check">
                                                    <input class="form-check-input @error('role_id') is-invalid @enderror"
                                                        ...
                                                        value="{{ $role->id }}"
                                                        {{ (old('role_id') ?? $selectedRoleId) == $role->id ? 'checked' : '' }}>
                            ...
                            @if(isset($rolePermissions) && $rolePermissions->isNotEmpty())
                                <div class="row mb-3">
                                    <label class="col-lg-2 col-form-label">Role Permissions:</label>
                                    <div class="col-lg-10">
